...
function Get-StoreQuerySpaceUsage
{
    param
    (
        [Parameter(Mandatory = $true)]
    ...
    else
    {
        ...
        $query = "SELECT TOP 5000 * FROM SpaceUsage"
        Write-Verbose $query

        $spaceUsage = Get-StoreQuery -Server $db.MountedOnServer -ProcessId $db.WorkerProcessId -Query $query -Unlimited
        while ($spaceUsage.Count % 5000 -eq 0)
        {
            $query = "SELECT TOP 5000 * FROM SpaceUsage WHERE TableName > '" + $spaceUsage[-1].TableName + "'"
            Write-Verbose $query
...
# Retrieves messages from specific full-text index completeness category (indexed, partially indexed, not indexed, corrupted, stale, should not be indexed)
function Get-StoreQueryBigFunnelIndexCompleteness
{
    [CmdletBinding()]
    param
    (
        ...
        [Parameter(Mandatory = $true)]
        [String]
        [ValidateSet("Indexed", "PartiallyIndexed", "NotIndexed", "Corrupted", "Stale", "ShouldNotBeIndexed")]
        $Category,
    ...
    else
    {
        ...
        # Retrieve mailbox number by mailbox guid
        [string]$query = "SELECT MailboxNumber FROM Mailbox WHERE MailboxGuid = '$MailboxGuid'"
        $mailboxObject = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        [string]$mailboxNumber = $mailboxObject.MailboxNumber
        if ([String]::IsNullOrEmpty($mailboxNumber))
        {
            Write-Error "Mailbox $MailboxGuid not found."
            return
        }
        
        # Retrieve conversations folder id to exclude it from analysis
        $query = "SELECT FolderId FROM Folder WHERE MailboxNumber = $mailboxNumber AND DisplayName = 'Conversations'"
        $conversationsFolderObject =  @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        $conversationsFolderId = $conversationsFolderObject.FolderId
        
        # Named full-text index related properties
        $query = "SELECT PropNumber from ExtendedPropertyNameMapping where MailboxNumber = $mailboxNumber and PropName = 'IsPartiallyIndexed'"
        $isPartiallyIndexedPropertyObject = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        $isPartiallyIndexedProperty = "p{0:x}000B" -f $isPartiallyIndexedPropertyObject.PropNumber
        
        $query = "SELECT PropNumber from ExtendedPropertyNameMapping where MailboxNumber = $mailboxNumber and PropName = 'IndexingErrorCode'"
        $indexingErrorCodePropertyObject = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        $indexingErrorCodeProperty = "p{0:x}0003" -f $indexingErrorCodePropertyObject.PropNumber
        
        $query = "SELECT PropNumber from ExtendedPropertyNameMapping where MailboxNumber = $mailboxNumber and PropName = 'IndexingErrorMessage'"
        $indexingErrorMessagePropertyObject = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        $indexingErrorMessageProperty = "p{0:x}001F" -f $indexingErrorMessagePropertyObject.PropNumber
        
        $query = "SELECT PropNumber from ExtendedPropertyNameMapping where MailboxNumber = $mailboxNumber and PropName = 'LastIndexingAttemptTime'"
        $lastIndexingAttemptTimePropertyObject = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        $lastIndexingAttemptTimeProperty = "p{0:x}0040" -f $lastIndexingAttemptTimePropertyObject.PropNumber
        
        $query = "SELECT PropNumber from ExtendedPropertyNameMapping where MailboxNumber = $mailboxNumber and PropName = 'IndexingAttemptCount'"
        $indexingAttemptCountPropertyObject = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        $indexingAttemptCountProperty = "p{0:x}0003" -f $indexingAttemptCountPropertyObject.PropNumber
        
        $query = "SELECT PropNumber from ExtendedPropertyNameMapping where MailboxNumber = $mailboxNumber and PropName = 'IsPermanentFailure'"
        $isPermanentFailurePropertyObject = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        $isPermanentFailureProperty = "p{0:x}000B" -f $isPermanentFailurePropertyObject.PropNumber
        
        # All full-text index related properties
        [string]$propertiesToFetch =
            "MessageId," + `
            "MessageClass," + `
            "Subject," + `
            ...
            "$isPartiallyIndexedProperty," + `
            "$indexingErrorCodeProperty," + `
            "$indexingErrorMessageProperty," + `
            "$lastIndexingAttemptTimeProperty," + `
            "$indexingAttemptCountProperty," + `
            "$isPermanentFailureProperty"
        
        [string]$topConstraint = [String]::Empty
        if (-not [String]::IsNullOrEmpty($Top))
        {
            $topConstraint  = "TOP $Top"
        }
        ...
        switch ($Category) 
        {
            "Indexed" 
            {
                $whereConstraint = "(BigFunnelPoiNotNeededReason = NULL or BigFunnelPoiNotNeededReason <= 0) and (BigFunnelPOISize > 0) and (BigFunnelPOIIsUpToDate = true) and ($isPartiallyIndexedProperty = null or $isPartiallyIndexedProperty = false)"
            }
            "PartiallyIndexed" 
            {
                $whereConstraint = "(BigFunnelPoiNotNeededReason = NULL or BigFunnelPoiNotNeededReason <= 0) and (BigFunnelPOISize > 0) and (BigFunnelPOIIsUpToDate = true) and $isPartiallyIndexedProperty = true"
            }
            "NotIndexed" 
            {
                $whereConstraint = "(BigFunnelPoiNotNeededReason = NULL or BigFunnelPoiNotNeededReason <= 0) and (BigFunnelPOISize = NULL or BigFunnelPOISize <= 0) and (BigFunnelPOIIsUpToDate = NULL or BigFunnelPOIIsUpToDate = false)"
            }
            "Corrupted" 
            {
                $whereConstraint = "(BigFunnelPoiNotNeededReason = NULL or BigFunnelPoiNotNeededReason <= 0) and (BigFunnelPOISize = NULL or BigFunnelPOISize <= 0) and (BigFunnelPOIIsUpToDate = true)"
            }
            "Stale" 
            {
                $whereConstraint = "(BigFunnelPoiNotNeededReason = NULL or BigFunnelPoiNotNeededReason <= 0) and (BigFunnelPOISize > 0) and (BigFunnelPOIIsUpToDate = NULL or BigFunnelPOIIsUpToDate = false)"
            }
            "ShouldNotBeIndexed" 
            {
                $whereConstraint = "(BigFunnelPoiNotNeededReason > 0)"
            }
        }
        
        $query = "SELECT $topConstraint $propertiesToFetch FROM Message WHERE MailboxNumber = $mailboxNumber AND FolderId != $conversationsFolderId AND $whereConstraint"
        $messages = @(Get-StoreQuery -Database $db -Query $query -Unlimited:$Unlimited)
        
        # Format output
        $messages = @($messages | Select-Object `
            @{Name = "MessageId"; Expression = {$_."MessageId"}}, `
            @{Name = "MessageClass"; Expression = {$_."MessageClass"}}, `
            @{Name = "Subject"; Expression = {$_."p0037001F"}}, `
            @{Name = "BigFunnelPOISize"; Expression = {$_."BigFunnelPOISize"}}, `
            @{Name = "BigFunnelPOIIsUpToDate"; Expression = {$_."p3655000B"}}, `
            @{Name = "BigFunnelPoiNotNeededReason"; Expression = {$_."p365A0003"}}, `
            @{Name = "IsPartiallyIndexed"; Expression = {$_."$isPartiallyIndexedProperty"}}, `
            @{Name = "IndexingErrorCode"; Expression = {$_."$indexingErrorCodeProperty"}}, `
            @{Name = "IndexingErrorMessage"; Expression = {$_."$indexingErrorMessageProperty"}}, `
            @{Name = "LastIndexingAttemptTime"; Expression = {$_."$lastIndexingAttemptTimeProperty"}}, `
            @{Name = "IndexingAttemptCount"; Expression = {$_."$indexingAttemptCountProperty"}}, `
            @{Name = "IsPermanentFailureProperty"; Expression = {$_."$isPermanentFailureProperty"}})
